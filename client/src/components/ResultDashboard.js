...
import { Activity, AlertTriangle, BarChart3, CheckCircle2, Download, Gauge, IndianRupee, Info, Percent, Printer, RefreshCw, ShieldAlert, Wallet, XCircle } from 'lucide-react';
...
function StatCard({ label, value, sub, color, icon, darkMode }) {
  const bg = 'premium-panel card-lift';
  return (
...
const TENURE_COLORS = ['#38BDF8', '#22C55E', '#F59E0B', '#8B5CF6'];
...
export default function ResultDashboard({ result, formData, darkMode, onNewAnalysis }) {
  ...
  const gradeColor = { A: '#22C55E', B: '#38BDF8', C: '#F59E0B', D: '#EF4444' }[riskGrade] || '#94A3B8';
  const displayDecision = decisionStatus || (approved ? 'Approved' : 'Rejected');
  const approvalColor = displayDecision === 'Approved' ? '#22C55E'
    : displayDecision === 'Conditional Approval' ? '#F59E0B'
      : '#EF4444';
    ...
    return (
      <div className="max-w-4xl mx-auto">
        <div className={`${cardBase} p-8 md:p-10 text-center`}>
          <div className="w-16 h-16 mx-auto mb-5 rounded-lg flex items-center justify-center"
            style={{ background: 'rgba(0,229,255,0.08)', border: '1px solid rgba(0,229,255,0.25)' }}>
            <BarChart3 size={24} color={darkMode ? '#38BDF8' : '#0369A1'} />
          </div>
          ...
          <button onClick={onNewAnalysis} className="btn-primary py-2.5 px-5 text-sm inline-flex items-center gap-2">
  ...
  const PIE_COLORS = ['#38BDF8', '#8B5CF6', '#22C55E'];
    ...
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <h2 className={`text-2xl md:text-3xl font-bold mb-1 ${textPrimary}`}>
        ...
        <div className="flex gap-3 no-print">
          <button onClick={onNewAnalysis} className={`px-4 py-2 rounded-lg text-sm font-semibold border transition-all duration-200
            ${darkMode ? 'border-[#1E2D4D] text-slate-300 hover:border-[#38BDF830] hover:text-[#38BDF8]' : 'border-gray-200 text-gray-600 hover:border-blue-300'}`}>
            <span className="inline-flex items-center gap-2"><RefreshCw size={15} /> New Analysis</span>
          </button>
          <button onClick={handleDownload} className="btn-primary py-2 px-4 text-sm">
            <span className="inline-flex items-center gap-2"><Download size={15} /> Download Report</span>
          </button>
          <button onClick={handlePrint} className={`px-4 py-2 rounded-lg text-sm font-semibold border transition-all
            ${darkMode ? 'border-[#1E2D4D] text-slate-300 hover:border-[#F59E0B40] hover:text-[#F59E0B]' : 'border-gray-200 text-gray-600'}`}>
            <span className="inline-flex items-center gap-2"><Printer size={15} /> Print</span>
          </button>
        </div>
      </div>

      {/* Approval Banner */}
      <div className={`premium-panel p-6 flex flex-col sm:flex-row items-center justify-between gap-4`}
        ...
        <div className="flex items-center gap-4">
          <div className="w-14 h-14 rounded-lg flex items-center justify-center"
            ...
            {displayDecision === 'Approved'
              ? <CheckCircle2 size={24} color={approvalColor} />
              : displayDecision === 'Conditional Approval'
                ? <AlertTriangle size={24} color={approvalColor} />
                : <XCircle size={24} color={approvalColor} />}
          </div>
          <div>
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="text-center">
            ...
            <div className={`text-sm font-semibold px-3 py-1 rounded-full`}
              style={{ background: `${riskLevel === 'Low' ? '#22C55E' : riskLevel === 'Medium' ? '#F59E0B' : '#EF4444'}15`,
                color: riskLevel === 'Low' ? '#22C55E' : riskLevel === 'Medium' ? '#F59E0B' : '#EF4444',
                border: `1px solid ${riskLevel === 'Low' ? '#22C55E' : riskLevel === 'Medium' ? '#F59E0B' : '#EF4444'}30` }}>
              {riskLevel}
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <StatCard label="Monthly EMI" value={`₹${animatedEMI.toLocaleString('en-IN')}`}
          sub={`${result.tenure || formData?.tenure || '-'} months`} color="#38BDF8" icon={<Wallet size={18} />} darkMode={darkMode} />
        <StatCard label="Eligible Limit" value={`₹${Number(eligibleLimit || 0).toLocaleString('en-IN')}`}
          sub="safe principal estimate" color="#22C55E" icon={<IndianRupee size={18} />} darkMode={darkMode} />
        <StatCard label="Affordability" value={`${animatedScore}`}
          sub="out of 100" color={affordabilityScore >= 75 ? '#22C55E' : affordabilityScore >= 50 ? '#F59E0B' : '#EF4444'} icon={<Gauge size={18} />} darkMode={darkMode} />
        <StatCard label="Recommended" value={`${recommendedTenure || formData?.tenure || '-'}M`}
          sub="safest tenure" color="#38BDF8" icon={<CheckCircle2 size={18} />} darkMode={darkMode} />
        <StatCard label="Interest Rate" value={`${annualRate}%`}
          sub="per annum" color="#8B5CF6" icon={<Percent size={18} />} darkMode={darkMode} />
        <StatCard label="Total Payable" value={`₹${(totalPayable / 1000).toFixed(1)}K`}
          sub={`+ ₹${(totalInterest / 1000).toFixed(1)}K interest`} color="#F59E0B" icon={<IndianRupee size={18} />} darkMode={darkMode} />
        <StatCard label="Remaining" value={`₹${animatedRemaining.toLocaleString('en-IN')}`}
          sub="post-EMI salary" color={remainingSalary > 0 ? '#22C55E' : '#EF4444'} icon={<Activity size={18} />} darkMode={darkMode} />
        <StatCard label="Risk Grade" value={riskGrade}
          sub={`${affordabilityBand || riskLevel} risk`} color={gradeColor} icon={<ShieldAlert size={18} />} darkMode={darkMode} />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className={`${cardBase} p-6 flex flex-col items-center justify-center`}>
          <h3 className={`flex items-center justify-center gap-2 text-xs font-semibold uppercase tracking-wider mb-4 ${textSecondary}`}>
            <Gauge size={14} /> Affordability Score
          </h3>
        ...
        <div className={`${cardBase} p-6`}>
          <h3 className={`flex items-center gap-2 text-xs font-semibold uppercase tracking-wider mb-4 ${textSecondary}`}>
            <BarChart3 size={14} /> Salary Allocation
          </h3>
        ...
        <div className={`${cardBase} p-6`}>
          ...
          <RiskBar label="Affordability Score" value={affordabilityScore}
            color={affordabilityScore >= 75 ? '#22C55E' : affordabilityScore >= 50 ? '#F59E0B' : '#EF4444'} darkMode={darkMode} />
          <RiskBar label="Income Utilization"
            ...
            color={((monthlyEMI + (formData?.existingEMI || 0)) / formData?.monthlyIncome) < 0.4 ? '#22C55E' : '#EF4444'} darkMode={darkMode} />
          <RiskBar label="Credit Health"
            value={Math.min(100, Math.round(((formData?.creditScore || 0) - 300) / 6))}
            color={formData?.creditScore >= 700 ? '#22C55E' : formData?.creditScore >= 600 ? '#F59E0B' : '#EF4444'} darkMode={darkMode} />
          <RiskBar label="Down Payment Ratio"
            value={Math.round((formData?.downPayment / formData?.productPrice) * 100)}
            color="#38BDF8" darkMode={darkMode} />
        </div>
      </div>
      ...
      <div className={`${cardBase} p-6`}>
        ...
        <ResponsiveContainer width="100%" height={260}>
          <BarChart data={repaymentTrend} margin={{ top: 5, right: 10, left: 0, bottom: 5 }}>
            ...
            <Bar dataKey="EMI" name="Monthly EMI" fill="#38BDF8" radius={[4, 4, 0, 0]} />
            <Bar dataKey="Interest" name="Total Interest" fill="#EF4444" radius={[4, 4, 0, 0]} />
          </BarChart>
        </ResponsiveContainer>
      </div>
      ...
      <div className={`${cardBase} p-6`}>
        ...
        <ResponsiveContainer width="100%" height={220}>
          <AreaChart data={repaymentTrend}>
            ...
            <Tooltip formatter={(v) => `₹${v.toLocaleString('en-IN')}`}
              contentStyle={{ background: chartBg, border: `1px solid ${gridColor}`, borderRadius: '8px', color: chartText }} />
            <Area type="monotone" dataKey="Total" name="Total Payable" stroke="#38BDF8" fill="url(#totalGrad)" strokeWidth={2} />
            <Line type="monotone" dataKey="EMI" name="Monthly EMI" stroke="#22C55E" strokeWidth={2} dot={{ fill: '#22C55E' }} />
          </AreaChart>
        </ResponsiveContainer>
      </div>
      ...
      <div className={`${cardBase} p-6`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {(noCostComparison || []).map((row, i) => (
            <div key={row.type || i} className={`p-4 rounded-lg border ${darkMode ? 'bg-[#0A0F1E] border-[#1E2D4D]' : 'bg-gray-50 border-gray-200'}`}>
              <div className="flex items-center justify-between mb-3">
                <span className={`text-sm font-semibold ${textPrimary}`}>{row.type}</span>
                {row.savings > 0 && <span className="text-xs font-semibold text-[#22C55E]">Saves ₹{row.savings.toLocaleString('en-IN')}</span>}
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <div className={`text-xs mb-1 ${textSecondary}`}>Monthly EMI</div>
                  <div className="font-mono font-bold" style={{ color: row.type === 'No Cost EMI' ? '#22C55E' : '#38BDF8' }}>
          ...
          <div className="space-y-3">
            {riskFactors?.map((factor, i) => (
              <div key={i} className={`flex items-start gap-3 px-3 py-2.5 rounded-lg
                ${factor.positive
                  ? (darkMode ? 'bg-[#22C55E08] border border-[#22C55E20]' : 'bg-green-50 border border-green-100')
                  : (darkMode ? 'bg-[#EF444408] border border-[#EF444420]' : 'bg-red-50 border border-red-100')}`}>
                <span className="text-sm flex-shrink-0 mt-0.5" style={{ color: factor.positive ? '#22C55E' : '#EF4444' }}>
                  {factor.positive ? <CheckCircle2 size={15} /> : <AlertTriangle size={15} />}
                </span>
        ...
        <div className={`${cardBase} p-6`}>
          ...
          <div className="space-y-3">
            {recommendations?.map((rec, i) => {
              const color = rec.type === 'success' ? '#22C55E' : rec.type === 'warning' ? '#F59E0B' : '#38BDF8';
              const bg = rec.type === 'success' ? '#22C55E08' : rec.type === 'warning' ? '#F59E0B08' : '#38BDF808';
              const border = rec.type === 'success' ? '#22C55E20' : rec.type === 'warning' ? '#F59E0B20' : '#38BDF820';
              return (
                <div key={i} className="flex items-start gap-3 px-3 py-2.5 rounded-lg"
                  style={{ background: bg, border: `1px solid ${border}` }}>
                  <span className="text-sm flex-shrink-0 mt-0.5" style={{ color }}>
                    {rec.type === 'success' ? <CheckCircle2 size={15} /> : rec.type === 'warning' ? <AlertTriangle size={15} /> : <Info size={15} />}
                  </span>
              ...
              {emiComparison?.map((row, i) => {
                ...
                return (
                  <tr key={i} className={`border-b transition-colors
                    ...
                    ${isCurrent ? (darkMode ? 'bg-[#38BDF808]' : 'bg-blue-50') : ''}`}>
                    <td className="py-3">
                      <span className={`font-mono font-semibold ${isCurrent ? (darkMode ? 'text-[#38BDF8]' : 'text-blue-600') : textPrimary}`}>
                        {row.tenure} months
                      </span>
                    </td>
                    <td className={`py-3 text-right font-mono ${textPrimary}`}>₹{row.emi?.toLocaleString('en-IN')}</td>
                    <td className="py-3 text-right font-mono text-[#EF4444]">₹{row.totalInterest?.toLocaleString('en-IN')}</td>
                    <td className={`py-3 text-right font-mono ${textPrimary}`}>₹{row.totalPayable?.toLocaleString('en-IN')}</td>
                    <td className="py-3 text-right">
                      <span className={`text-xs px-2 py-0.5 rounded-full ${row.affordable ? 'bg-[#22C55E20] text-[#22C55E]' : 'bg-[#EF444420] text-[#EF4444]'}`}>
                        {row.affordable ? 'Yes' : 'No'}
                      </span>
                    </td>
                    <td className="py-3 text-right">
                      {isCurrent && <span className={`text-xs px-2 py-0.5 rounded-full ${darkMode ? 'bg-[#38BDF820] text-[#38BDF8]' : 'bg-blue-100 text-blue-700'}`}>Selected</span>}
                      {row.tenure === recommendedTenure && !isCurrent && <span className="text-xs px-2 py-0.5 rounded-full bg-[#22C55E20] text-[#22C55E]">Optimal</span>}
                    </td>
                  </tr>
                );
              })}
