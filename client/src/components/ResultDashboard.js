...
export default function ResultDashboard({ result, formData, darkMode, onNewAnalysis }) {
  ...
  const {
    ...
    recommendations, riskFactors, emiComparison, financialHealth,
    rejectionReasons
  } = result;
  ...
  const approvalColor = approved ? '#00FF87' : '#FF3D71';
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      <div className={`rounded-2xl p-6 flex flex-col sm:flex-row items-center justify-between gap-4`}
        style={{ background: approved ? 'rgba(0,255,135,0.06)' : 'rgba(255,61,113,0.06)', border: `1px solid ${approvalColor}30` }}>
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <div className={`text-sm ${textSecondary}`}>
              {approved ? 'You qualify for BNPL financing' : rejectionReasons?.join(', ') || 'Eligibility criteria not met'}
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        ...
        <StatCard label="Risk Grade" value={riskGrade}
          sub={riskLevel + ' risk'} color={gradeColor} icon="⟁" darkMode={darkMode} />
      </div>
                ...
                return (
                  <tr key={i} className={`border-b transition-colors
                    ...
                    <td className="py-3 text-right">
                      ...
                      {row.tenure === 6 && !isCurrent && <span className="text-xs px-2 py-0.5 rounded-full bg-[#00FF8720] text-[#00FF87]">Optimal</span>}
                    </td>
                  </tr>
                );
              })}
